...
metadata_random<-readRDS("../data/metadata_random.rds") %>% 
...
H.PredLife<-readRDS("../data/H.PredictedAgeData.rds")
...
ggsave(plot = sampledis, filename = "SampleDistribution.png",dpi=300)
```

# Fig 2
```{r fig.height=8,fig.width=16}
relab_phylum_long<-readRDS("../data/relabundance_long_phyla_nochl.rds") %>% 
...
age_gaussian_quad<-readRDS("../data/FeatureAnalysis_LMM_L1O_gaussian_quad_meancentered_May21.rds")
...
silva<-readRDS("../data/ASVs_silva_assigned_extendedMay21.rds") %>% 
...
ordination<-readRDS("../data/ordination_data_bstatus0.rds") %>% 
  select(DADA_id, starts_with("PC")) %>% 
  select(-c(PC3,PC9,PC10))
colnames(ordination)<-c("DADA_id",paste("PC ",seq(1:2),"*",sep=""),paste("PC ",4:8,"*",sep=""))

cow_metadata<-readRDS("../data/metadata_bstatus0_mathormonerank_rainpriortomilestone_matrankbirthgroup.rds") %>% 
  rename(age_chrono=age.years) %>%
...
duck_metadata<-readRDS("../data/metadata_bstatus0_mathormonerank_rainpriortomilestone_matrankbirthgroup.rds") %>% 
...
ranks<-readRDS("../data/rank_raw_adult_Feb2021.rds") %>%
  select(sname,collection_date,collect_grp,ordrank,proprank)
ranks$collect_grp<-as.factor(ranks$collect_grp)
PredLifeCov<-readRDS("../data/metadata_slopes_life_and_breakpoints.rds") %>% 
