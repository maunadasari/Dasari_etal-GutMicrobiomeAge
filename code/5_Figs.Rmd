...
#file with just the data that actually went into my model 
...
#ggsave(plot = sampledis, filename = "../Figures/SampleDistribution.png",dpi=300)
```
...
#ggsave(plot=plots,filename = "../Figures/2A_GauProPlot.png",width=11,height=5,dpi=300,units="in")
```
...
estimatesageaccel<-ggplot(data=H.PredLife, aes(x=age_chrono,y=age_mb))+
  ...
  xlab(bquote(Chronological~Age~In~Years~(age[c])))+
  ylab(bquote(Predicted~Microbiome~Age~In~Years~(age[m])))+
  ggtitle(expression(paste("Measuring Microbiome ", Delta, "age")))+
  ...
  annotate(geom = "text", x = 7, y = 15.25, size=6,
           label = bquote(`(`~8~`=`~14~`-`~6~`)`),color="red")+
  annotate(geom = "text", x = 4, y = 17.25, size=6, lineheight = .9,
  ...
  annotate(geom = "text", x = 8, y = 23, size=6,
           label = bquote(Delta~age~`=`~Age[m]~`-`~Age[c]), hjust = "left",color="black")+labs(tag="C")+
  theme(axis.title=element_text(size=18),
        ...
        legend.position = c(.2,.85), 
        legend.title=element_blank(),
        plot.tag = element_text(size=20));estimatesageaccel
#grid.arrange(estimatesageaccel, estimatespace, ncol=2)
#plots<-arrangeGrob(estimatesageaccel,estimatespace,ncol=2)
#ggsave(plot=plots,filename="../Figures/4A_ConceptualMetrics.png",width=11,height=5,dpi=300,units="in")
```
...
sheep=ggdraw()+
  ...
  draw_plot(estimatesageaccel, x=0.3, y=0, width=.5,height=0.5)

sheep
  
#ggsave(plot=sheep,filename="Fig3.pdf",width = 14, height = 14,units="in")
```
...
p.females<-ggplot(data=females,
  ...
  ylab(expression(paste("Corrected Microbiome ", Delta, "age")))+
  theme_bw()+
...
plots<-(p.females+p.males)
plots
ggsave(plot=plots,filename="Fig4.pdf",height=11,width=11, units="in", dpi=300)
```
```{r fig.height=11,fig.width=11}

PredLifeCov_F2<-females %>% mutate(season=if_else(season=="wet","Wet","Dry"))
season_females<-ggplot(data=PredLifeCov_F2,
  ...
  theme_bw()+
  xlab("Season")+
  ylab(expression(paste("Microbiome ", Delta, "age")))+
...
#ggsave(plot=plots2,filename="4D_socioenviroplots.png")
```
...
AllSigFeatures$Feature_Category[AllSigFeatures$Feature_Category=="family"]<-"Family"
AllSigFeatures$Feature_Category[AllSigFeatures$Feature_Category=="genus"]<-"Genus"
feature_levels<-c("Alpha Diversity","Composition", "Phylum", "Family", "Genus", "ASV")
AllSigFeatures$Feature_Category<-factor(AllSigFeatures$Feature_Category, 
                                              levels=feature_levels)
...
plot6<-plot4/plot5
plot6
#ggsave(plot=plot6,filename = "../Figures/LMMFeatures2.png",height=7, width=10,units="in",dpi=300)
```
...
ENR<-readRDS("../data/MLoutput_ENR.rds") 
...
RF<-readRDS("../data/MLoutput_RF.rds")
...
GPRonly<-readRDS("../data/MLoutput_GPRonly.rds")
...
GPRHet<-readRDS("../data/HetGPR.PredictedAgeData.rds")
...
#knitr::include_graphics("../Figures/graph_ENR_alphatrials.png")
```
...
AllTaxa<-readRDS("../data/GPRL1O_AllTaxaTable.rds") %>%  #all ready to go, but has pub ready names
